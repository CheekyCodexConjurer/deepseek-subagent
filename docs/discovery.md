...
- A disposable `codex app-server --listen ws://127.0.0.1:45558` accepted a WebSocket handshake; the bridge transport completed `initialize`, and a direct protocol smoke on that same disposable server completed `thread/start`. This was a separate process and not the Desktop connection.
- A disposable app-server initialize handshake succeeded through the bridge adapter. A command-line `-c` override did not expose a new MCP server, but an isolated temporary `CODEX_HOME` containing `[mcp_servers.deepseek_subagent]` did: `mcpServerStatus/list` returned serverInfo `deepseek-subagent`, title `DeepSeek Sub-Agent`, and all five bridge tools in the pre-delivery inventory. The current contract now registers seven tools, including consult and follow.
- On an ephemeral thread in that isolated app-server, `mcpServer/tool/call` invoked `deepseek_spawn` and returned `accepted=true` with a structured `jobId` in about the expected immediate path. The direct RPC did not emit an `item/completed` notification, so this is evidence of MCP exposure and structured identity only, not proof of model-originated item correlation. The historical inventory predates `consult` and `follow`; the current bridge contract registers seven tools and its live suite verifies the new event-driven path through the daemon.
...
- The bridge adapter now supports an explicitly configured loopback `ws://` endpoint using the Node WebSocket API. No such endpoint was active in the Desktop process during discovery, so this transport does not change the current Windows Desktop capability result.
- The WebSocket transport accepts the installed App Server's response/notification envelope shape (the local server omits `jsonrpc` on observed responses), but requires a valid result/error response and rejects malformed or unknown request envelopes. The endpoint has no separate authentication in this bridge and is therefore documented as same-user loopback only.
- The installed Codex CLI accepts MCP server configuration in `config.toml`; the bridge installer writes `tool_timeout_sec = 4500` only in the `deepseek-subagent` section after creating a backup. The current user config was not modified by this delivery.
...
- The busy-session behavior of the installed OpenCode version was not run in this delivery's opt-in live suite. The implementation therefore handles a busy `prompt_async` response conservatively by aborting only the active turn and resubmitting to the same session; a live smoke is still the proof for a specific installation.
