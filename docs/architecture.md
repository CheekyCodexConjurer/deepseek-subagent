...
`deepseek_spawn` accepts an optional `model_route`; it must name the route that is currently active. An unknown or disabled route fails closed with a stable typed 400 before any workspace, worktree, session or job side effect; any other registered route is denied with a typed 403 `route_override_denied`, because route changes are operator-only — an explicit route matching the active route is accepted for compatibility. There is no fallback route, and a `model_route` never changes the active pointer.
...
All errors crossing the HTTP or MCP boundary are typed: a stable machine-readable code plus an explicit HTTP status (400 invalid request, 401 unauthorized, 403 route override denied, 404 missing resource, 409 state conflict, 500 internal). Route selection is operator-controlled: `deepseek_spawn` callers may only name the route that is currently active — an unknown or disabled route fails closed with a typed 400 (`unknown_route` / `route_disabled`) and any other registered route with a typed 403 `route_override_denied`, all before any workspace or session side effect, with no fallback route. HTTP bodies carry `{ error, code, status }` plus `details`, and, for busy errors, `retry: false` and the conflicting `jobId`; the MCP layer propagates the same code and `details` through `structuredContent` and additionally marks `retry: false` for the non-retryable statuses 409 (busy) and 403 (route override denied). Consumers branch on the code, never on message text.
